...
void inserir_aresta_m(M_GRAFO* graf, int d, int row, int col, int peso){

	graf->digraf = d;
	if (graf->digraf == 1){ // se for digrafo, apenas uma atribuição a matriz será feita, em decorrencia de haver orientação no grafo
		graf->adj_mat[row][col] = peso;
	
	}else{ 
		graf->adj_mat[row][col] = peso; //se for um grafo não valorado , o valor atribuido(peso) a matriz só poderá ser 0 ou 1
		graf->adj_mat[col][row] = peso; 
	}						 
}
