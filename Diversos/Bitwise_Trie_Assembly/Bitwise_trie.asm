...
main:
	li $a1, 2			# Passando 2 como argumento em a1 para que o no raiz seja criado
	...
	sw $t0, 12($s1)			# setando o node_val da raiz como 2, para indicar que o no eh raiz. 
...
main_loop:
	...
	move $t0, $v0
    	move $s4, $t0			# guardar a opcao escolhida em um registrador s4 para uso futuro 
...
read_str:
   	...
   	la $a0, binary_number 		# A string digitada fica salva em binary_number . O $a0 tem o endereco base para a string alocada dinamicamente.
   	li $a1, 16		
    	syscall
...
TenToZero:			# TenToZero faz a substituicao do 10 == ENTER por mais um \0 
...
end_checker_loop:
	...
	move $s0, $a0 		# No caso de o numero digitado ser valido, ele sera salvo em um registrador s0				
		      		# Ate aqui o endereco que é guardado por a0, tem seu conteudo alterado 
...
create_node:	
	...
	sw $v0, 0($s0)			# Salva o conteudo de v0 no nó da esquerda
					# linka o no atual com o novo no criado
...
create_node_dir:		
	...
	sw $v0, 4($s0)		# linka o no atual com o novo no criado
...
insert:		
	...
	li $t6, 1		# t6 é usado para setar os niveis para os nos que forem criados						

	jal search_repeated
	beq $v0, 1, repeated_node		
	bgezal $s0, insert_loop # Se o valor contido em s0 for maior ou igual a zero , pode-se criar um no valido
...
insert_loop:	
	...
	li $a1, 1			# Passando 1 como argumento para que seja criado um no a direita do no atual
	la $a2, ($t6)
	bgezal $t5, create_node		# Se t0 == 0, node nao existe entao crie. Se t0 < 0, node existe.
									
	lw $t1, 4($t1)
	
	addi $t0, $t0, 1		# Acessar a proxima posicao da string do numero binario
	addi $t6, $t6, 1
...
insert_left:
	...
	move $a0, $t1			# Passando o no atual como parametro em $a0
...
end_insert_loop:
	...
	sw $t5, 8($t1) 			# O true será atribuído ao terminador no ultimo no que sera inserido
...
search_repeated:
	...
	beq $t5, 48, node_left_sr	# Se t2 == 48 (caractere 0 em ascII) , va para a o no da esquerda
	beq $t5, 49, node_right_sr	# Se t2 == 49 (caractere 1 em ascII), va para a o no da direita
...
search_loop_sr:		# BUSCAR CHAVE REPETIDA ANTES DA INSERCAO
	...
	beq $t9, $zero, term_check_sr	# Condicao de parada. Verificacao do terminador
...
search: 
   	...
   	move $t0, $s0			# t0 guarda temporariamente o endereco para a string. 
	move $t1, $s1			# O mesmo ocorre com t1, que agora recebe o endereco para a raiz da arvore
   	move $t9, $s2			# t9 guarda temporariamente o endereço para a string do caminho percorrido


	li $t6, 50			# Colocando o valor 2 ( 50 == 2 em ascII ) na primeira posicao da string do caminho percorrido,
	sb $t6, 0($t9)			# para indicar que passamos pela raiz
	addi $t9, $t9, 1		# Acessando a proxima posicao da string do caminho percorrido

	lb $t2, 0($t0)			# Guardando o caractere atual do numero binario em t2
	
	beq $t2, 48, node_left		# Se t2 == 48 (caractere 0 em ascII) , va para a o no da esquerda
	beq $t2, 49, node_right		# Se t2 == 49 (caractere 1 em ascII), va para a o no da direita
	j search			
...
end_search_loop:
	...
	li $v0, 1
	beq $s4, 2, remove		# Usa o conteudo de s4 setado no inicio do programa para retornar para a funcao correta
	beq $s4, 3, search
	
	
not_found:
	
	jal end_path			# Coloca uma flag na ultima posicao da string 
...
left_path:
	...
	li $t6, 48			# Guardando o valor 0, que indica que fui pra esquerda , na posicao atual de t9
	sb $t6, ($t9)			
	addi $t9, $t9, 1
	jr $ra
...
remove: 
	...
	addi $t9, $t9, 1		# Ir para a proxima posicao da string do caminho percorrido
...
print_tree:
	...
	subi $sp, $sp, 4
	sw $s1, 0($sp) 			#enfileira o no raiz
...
loop_visu:
	...
	lw $t6, 16($t2) 		#t6 recebe o nivel do no atual
	
	
	beq $t3, 0, visu_enfileirar_dir #pular se t3 == NULL
	subi $sp, $sp, 4
	sw $t3, 0($sp) 			#enfileira filho da esquerda
	
visu_enfileirar_dir:
	beq $t4, 0, visu_mudar_nivel 	#pular se t4 == NULL
	subi $sp, $sp, 4
...
visu_impressao:
	la $a0, ($t2)			# Passando o nó atual como parametro para a funcao print_node
...
node_address:
	...
	lw $t3, 0($t9) # impressao do endereco do no esquerdo
	li $v0, 1
	la $a0, ($t3)
	syscall
	...
	lw $t3, 4($t9)	# impressao do endereco do no direito
	li $v0, 1
	la $a0, ($t3)
	syscall
